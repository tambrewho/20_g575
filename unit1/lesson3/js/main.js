...
function addColumns(cityPop) {
	$('tr').each(function(i) {
    	...
    	$(this).append('<th>City Size</th>');
			...
			// variable to hold each ciy's respective size
  		var citySize;
...
function debugCallback(response){
	// define mydata for append method
	mydata = response;

	// jquery appends each row of the json data to the div while converting it to a string
	$(mydiv).append(JSON.stringify(mydata));
};

// debugAjax function
function debugAjax(){
	var mydata; // var for holding cities JSON data

	$.ajax("data/MegaCities.geojson", { // use jquery to select data from file link
		dataType: "json",
		success: function(response){

			mydata = response;
			// call debugCallback function after success
			debugCallback(response);
		}
	});

	// jquery selects the div and adds a line break and the following title
	$(mydiv).append('<br>GeoJSON data:</br>');
};

//call the initialize and debugAjax function when the document has loaded
